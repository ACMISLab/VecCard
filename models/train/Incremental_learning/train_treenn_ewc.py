...
import model.treenn.treenn as TNN_EWC
import argparse
...
def main():
    ...
    model = TNN_EWC.TreeNN(input_dim, args.embedding_hidden_dim,
                                   args.estimation_hidden_dim, args.dropout)
    ...
    for epoch in range(num_epochs):
        ...
        for trees_batch, labels_batch in train_dataloader_b:
            ...
            predictions = torch.stack(predictions).squeeze()
            labels_batch = torch.tensor(labels_batch, dtype=torch.float32)
            loss = criterion(predictions, labels_batch) + ewc.ewc_loss(model, args.weight, estimated_fisher, estimated_mean)
            loss.backward()
            optimizer.step()
