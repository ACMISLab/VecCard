...
import model.treernn.treernn as TRNN_EWC
...
def main():
    ...
    # task b
    train_trees_b, train_labels_b = FeatureData.TreeDataset('task_3_train',
                                                            args.dataset_count, ds_args).loadData()
    test_trees_b, test_labels_b = FeatureData.TreeDataset('task_3_test',
                                                          args.dataset_count, ds_args).loadData()
    ...
    model = TRNN_EWC.QueryRNN(input_dim, args.mem_dim, args.hidden_dim)
    ...
    print("=====================> start train task a <=====================")
    for epoch in range(num_epochs):
        model.train()
        total_loss = 0
        for trees_batch, labels_batch in train_dataloader_a:
            trees_batch = [tree.to(device) for tree in trees_batch]
        ...
        average_loss = total_loss / len(train_dataloader_a)
    ...
    estimated_mean, estimated_fisher = ewc.estimate_ewc_params(model, train_dataset_a, args.batch_size)
    ...
    for epoch in range(num_epochs):
        ...
        for trees_batch, labels_batch in train_dataloader_b:
            ...
            predictions = torch.stack(predictions).squeeze()
            labels_batch = torch.tensor(labels_batch, dtype=torch.float32)
            loss = criterion(predictions, labels_batch) + ewc.ewc_loss(model, args.weight, estimated_fisher,
                                                                       estimated_mean)
            loss.backward()
            optimizer.step()
