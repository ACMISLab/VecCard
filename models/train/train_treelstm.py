...
def main():
    ...
    val_y_dataloader = DataLoader(val_y_dataset, batch_size=args.batch_size, shuffle=False,
                                  collate_fn=FeatureData.collate_fn)
    first_available_tree = next(iter(m_test_trees or y_test_trees), None)
    ...
    if args.retrain:
        ...
        if not train_trees:
            return
        print(
            f"====> 训练数据集加载完毕，共 {len(train_labels)} 条数据, 最大值: {torch.max(train_labels).item()}, "
            f"最小值: {torch.min(train_labels).item()}")
        ...
        if model_path.exists() and first_available_tree is not None:
            input_dim = first_available_tree.getChildNode()[0].shape[0]
            print(f'模型输入纬度 {input_dim}')
            model = TLSTM.TreeLSTM(input_dim, args.embedding_hidden_dim, args.lstm_hidden_dim,
                                   args.estimation_hidden_dim, args.attention, args.dropout)
...
def parse_args():
    ...
    parser.add_argument('--dropout', default=0.5,
                        help='dropout')
    parser.add_argument('--input_dim', default=166,
                        help='input_dim')
