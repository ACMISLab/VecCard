...
class ChildSumTreeRNN(nn.Module):
    ...
    def forward(self, operator, inputs):
        ...
        if operator == 'and':
            output = self.and_op(inputs)
        elif operator == 'or':
            output = self.or_op(inputs)
        return output
...
class QueryRNN(nn.Module):
    ...
    def __init__(self, input_dim, mem_dim, hidden_dim):
        super(QueryRNN, self).__init__()
        self.mlp = MLP(input_dim, mem_dim)  # 多层感知机
        self.treelstm = ChildSumTreeRNN(mem_dim, mem_dim)  # 树形RNN
        self.regressor = RegressorNN(mem_dim, hidden_dim)  # 回归预测
        # device
    ...
    def process_tree(self, tree):
        ...
        if tree.haveChildNode():  # array
            for child_node in tree.getChildNode():
                ...
                leaf_output = self.mlp(leaf_features)
                outputs.append(leaf_output)
        inputs = torch.stack(outputs)
        out = self.treelstm(tree.getOperator(), inputs)
        return out
